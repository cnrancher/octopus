...
// Borrowed from sigs.k8s.io/controller-runtime/pkg/envtest/crd.go
...
func (options CRDInstallOptions) Install(config *rest.Config) error {
	defaultCRDOptions(&options)

	if err := readCRDFiles(&options); err != nil {
		return err
	}
	if err := options.create(config); err != nil {
		return err
	}
	if err := options.wait(config); err != nil {
		return err
	}
...
func (options CRDInstallOptions) Cleanup(config *rest.Config) error {
	if !options.CleanUpAfterUse {
		return nil
	}
...
func (options *CRDInstallOptions) create(config *rest.Config) error {
...
func (options *CRDInstallOptions) wait(config *rest.Config) error {
	if len(options.CRDs) == 0 {
		return nil
	}
