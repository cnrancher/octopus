...
// Borrowed from sigs.k8s.io/controller-runtime/pkg/envtest/webhook.go
...
func (o *WebhookInstallOptions) generateHostPort() (string, error) {
	port, err := fuzz.FreePort()
	if err != nil {
		return "", fmt.Errorf("unable to grab random port for serving webhooks on: %v", err)
	}
	o.LocalServingPort = port
	o.LocalServingHost = "127.0.0.1"
	return net.JoinHostPort(o.LocalServingHost, fmt.Sprintf("%d", port)), nil
}
